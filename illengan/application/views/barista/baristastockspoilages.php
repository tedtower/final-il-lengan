...
<script>
 $(document).ready(function() {
	...
	function createPagination(pageNum){
		$.ajax({
			url: '<?=base_url()?>barista/loadDataSpoilagesStock/'+pageNum,
			type: 'get',
			dataType: 'json',
	...
	function setSpoilagesData(table) {
        ...
        for(t in table){
			var row =`<tr class="spoilagesTabletr ic-level-1" data-tiActual="`+table[t].tiActual+`" data-stQty="`+table[t].stQty+`" data-tiRemarks="`+table[t].tiRemarks+`" data-tiDate="`+table[t].tiDate+`" data-stID="`+table[t].stID+`" data-siID="`+table[t].siID+`">`;
