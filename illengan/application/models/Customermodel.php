...
class Customermodel extends CI_Model
{
    ...
    function orderInsert($total, $tableCode, $orderlist, $customer, $dateTime)
    { //insert in table orderslip
        ...
        foreach ($orderlist as $items) {
            $prID = $items['id'];
            $sql = "SELECT CONCAT(mName, ' ', '(',prName,')', IF(mTemp IS NULL,' ',CONCAT(' ',mTemp))) as prName, prPrice
            ...
            foreach ($rest as $r) {
                ...
                $this->db->query($query2, array(NULL, $order_id, $items['id'], $r['prName'], $items['qty'], $items['subtotal'], 'pending', $items['remarks'], $r["prPrice"], ''));
                ...
                if (!empty($addOns)) {
                    ...
                    for (
                        $i = 0, $q = 0, $s = 0;
                        $i < count($addonIds), $q <  count($addonQtys), $s <  count($addonSubtotals);
                        $i++, $q++, $s++
                    ) {
