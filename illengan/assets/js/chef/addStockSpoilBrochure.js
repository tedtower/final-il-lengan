function getSelectedStocks() {
    ...
    for (var i = 0; i <= choices.length - 1; i++) {
        if (choices[i].checked) {
            ...
            $.ajax({
                type: 'POST',
                url: 'http://www.illengan.com/chef/viewStockJS',
                data: {
                    stID : value
                },
                dataType: 'json',
                ...
                success: function (data) {
                    ...
                    stockChecked = `<tr class="stockelem" data-id="` + data[i].stID + `" >
                            ...
                            <td><input type="text" id="stName` + i + `" name="stName"
                                    class="form-control form-control-sm"  value="` + data[i].stName + `"  required disabled></td>
...
function addStockItems() {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://www.illengan.com/chef/spoilages/stock/add',
        data: {
